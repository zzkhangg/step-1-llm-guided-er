...
\bottomrule
\end{tabular}}
\caption{DBLP--ACM, 2{,}224 ground-truth matches. Every row is a single matcher draw
recorded 12--19 August; see the correction in Section~\ref{sec:correction}, which repeats
the first and fourth rows and finds these levels no longer reproduce.}
\end{table}
...
\subsection{Correction: these levels do not reproduce}
\label{sec:correction}

The table above reports one call of the matcher per pair. Re-running its two headline
configurations on 26--27 August, three times each with the response cache disabled so that
each run is an independent draw, does not reproduce the levels:

\begin{table}[h]
\centering
\begin{tabular}{lrrrrr}
\toprule
Configuration & Draw 1 & Draw 2 & Draw 3 & Mean & Reported above \\
\midrule
Step 1.5, decisive (\texttt{gap}) & 0.9228 & 0.8886 & 0.9151 & 0.9088 & 0.9397 \\
No selection                      & 0.7632 & 0.7006 & 0.7047 & 0.7228 & 0.8706 \\
\bottomrule
\end{tabular}
\caption{DBLP--ACM F1 over three independent draws per configuration.}
\end{table}

Nothing in the pipeline changed between the two periods: the model slug, the prompt hash,
the attributes selected per pair and the token counts per prompt are identical. What changed
is the matcher. Against 2{,}224 true matches it asserted 2{,}867 in August and
3{,}591--4{,}111 now, so it has become markedly more willing to answer Yes, and precision
falls accordingly --- from 0.7729 to 0.5674 for no selection. OpenRouter serves one model
slug from several providers differing in quantization, and that mix is not fixed over time.

The comparison this report is about survives, and is larger than first measured. The two
ranges do not overlap: the worst adaptive draw, 0.8886, still exceeds the best no-selection
draw, 0.7632. The separation between the means is 0.186 against the 0.069 implied by the
single draws above, and the paired test within each draw gives adaptive selection 1{,}217,
1{,}689 and 1{,}677 uniquely correct pairs against 207, 341 and 224 ($p < 10^{-170}$ in all
three).

Two consequences for the rest of this report. Every F1 quoted anywhere in it is a
measurement of one matcher on one date rather than a property of the method, and a reader
reproducing these runs should expect the levels to move while the ordering holds. And the
adaptive configuration varies by 0.034 across draws against 0.063 for no selection: showing
the matcher fewer fields leaves it less to be swayed by, so condensation buys stability as
well as accuracy. That was not a designed property and is reported here as an observation.
...
\begin{tabular}{llrrrrrrr}
\toprule
Method & Rule & Attrs/pair & Tokens & TP & FP & FN & F1 & Precision \\
\midrule
No selection (\texttt{full}) & --- & 12.00 & 1035 & 1010 & 1018 & 133 & \textbf{0.6370} & 0.4980 \\
Step 1.5, decisive & \texttt{gap}, $k\!\ge\!1$ & 1.34 & \textbf{248} & 914 & \textbf{919} & 229 & 0.6142 & \textbf{0.4986} \\
Step 1.5, decisive & \texttt{cumulative} & 2.24 & 286 & 954 & 1273 & 189 & 0.5662 & 0.4284 \\
Step 1.5, decisive & \texttt{gap}, $k\!\ge\!2$ & 2.39 & 316 & 961 & 1413 & 182 & 0.5465 & 0.4048 \\
Step 1.5, ordinal & \texttt{gap}, $k\!\ge\!1$ & 2.18 & 382 & 979 & 1494 & 164 & 0.5412 & 0.3956 \\
Step 1, supervised & \multicolumn{8}{l}{did not run: a fixed cut-off selected no attributes} \\
\bottomrule
\end{tabular}
...
Precision as a function of width is therefore not monotone: 0.4986 at 1.34 attributes,
0.4048 at 2.39, and 0.4980 again at 12. One attribute gives the matcher a strong identifier
and little else; twelve give it contradicting fields --- price, dimensions --- with which to
refuse. Two give it neither, and the interval between 2.4 and 12 attributes is not yet
measured.
...
\textbf{The middle of the width--accuracy curve is unmeasured.} Amazon--Walmart precision is
0.4986 at 1.34 attributes per pair, 0.4048 at 2.39, and 0.4980 at 12. Whether it recovers
gradually across the intervening range or only once contradicting fields are present is not
established, and the answer decides whether a configuration cheaper than the full
representation exists that does not pay the recall cost. Nothing in this report resolves it.

\textbf{Single matcher draw, and the draw moves.} Each configuration was run once, and
Section~\ref{sec:correction} shows the level of a repeated configuration shifting by up to
0.15 over two weeks with nothing in the pipeline changed. The paired design makes the
comparisons valid within that draw, but does not establish that effect sizes replicate under
a different one. The Amazon--Walmart rows deserve the most caution: several differences there
are smaller than the 0.06 spread the repeated DBLP--ACM runs exhibited, and none of them has
been repeated. The large DBLP--ACM effects are unlikely to reverse; the
