...
not covered; it is running now and is the subject of the next report.
...
\textbf{Amazon--Walmart is missing.} It is the widest schema available (13 attributes) and
the least saturated baseline observed, so it is where adaptive selection should show its
largest effect and where the near-unique-key caveat above does not apply. Runs are in
progress; its absence is the main gap in the evidence.
...
\begin{enumerate}
\item Complete the Amazon--Walmart runs and repeat this comparison on a wide schema with no
      near-unique key. This is the decisive test of whether the result generalizes.
